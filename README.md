...
A varying coefficient model is considered, where the regression coefficients vary across samples according to the modulator. Based on this model, the response is generated with additive Gaussian noise.
...
For each target sample, the hyperparameter combination with the smallest `ssGCV` value is selected. Using the selected hyperparameters, a kernel-based elastic net model is then fitted.
